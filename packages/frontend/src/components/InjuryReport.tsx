...
const INJURY_KEYWORDS = ["injury", "injured", "out", "doubtful", "fitness", "muscle", "hamstring", "knock", "strain", "fracture", "surgery", "ruled out", "unavailable", "return", "recovery", "ankle", "knee", "thigh", "calf", "shoulder"];
          ...
          return (
            <div key={article.contentId} style={{ background: "#1e3a5f", borderRadius: "8px", padding: "0.75rem 1rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem" }}>
                ...
                <span style={{ background: STATUS_COLORS[status], color: "white", padding: "2px 10px", borderRadius: "12px", fontSize: "0.8rem", fontWeight: "bold", whiteSpace: "nowrap" }}>{status}</span>
              </div>
              <p style={{ margin: "0.25rem 0 0 0", fontSize: "0.85rem", color: "#9CA3AF" }}>{article.sourceName} · {article.publicationDate ? new Date(article.publicationDate).toLocaleDateString() : ""}</p>
            </div>
          );
        })}
      </div>
      <p style={{ fontSize: "0.8rem", color: "#6B7280", marginTop: "0.75rem" }}>Injury updates sourced from BBC Sport, Sky Sports, Guardian and Arsenal blogs.</p>
    </section>
  );
}
